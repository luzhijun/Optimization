...
def cmaUser(func):
    ...
    return {'param':es.result()[0],'time':time.time()-t1,'iter':es.countiter}
...
def cmaUser1(func):
    ...
    print 'eval value:%s'%es.result()[1]
    return {'param':es.result()[0],'matrix':mat,'time':time.time()-t1,'iter':es.countiter}
...
def compute():
    ...
    for i in [25]:
        ...
        S[i]=[res,res1,res2,res3]
    md.dumpData(S,'result1.tl')
    print 'finished!'
